...
class ASRecord( object ):
    ...
    def dbCalcSecuRiskStd(self):
        dbCursor = self.db.cursor()
        dbCursor.execute( 'SELECT SUM((secuRisk - (SELECT AVG(secuRisk) FROM asn where secuRisk>0 ))* (secuRisk - (SELECT AVG(secuRisk) FROM asn where secuRisk>0)) ) / (COUNT(secuRisk) - 1) AS Variance FROM asn where secuRisk>0')
        rows=dbCursor.fetchall()
        for row in rows:
            (stdRisk,) = row
        return stdRisk
